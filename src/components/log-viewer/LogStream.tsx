...
export function LogStream({ source }: LogStreamProps) {
  const { data: logs } = useLogs(source.toLowerCase());
  ...
  return <section className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm">
    <div className="border-b border-border bg-muted/30 px-4 py-3">
      <div className="flex items-center justify-between gap-2">
        <div><h2 className="font-medium">{source}</h2><p className="text-xs text-muted-foreground">{filtered.length} visible {filtered.length === 1 ? "entry" : "entries"}</p></div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <Input value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="Search messages or levels" className="min-w-44 flex-1" aria-label={`Search ${source} log`} />
    ...
    <ScrollArea className="min-h-0 flex-1"><div className="min-w-[38rem] font-mono text-xs">
      {filtered.length === 0 ? <Empty text={logs?.length ? "No entries match the current filters." : "No log entries have been recorded yet."} /> : <>{filtered.map((line: LogLine, index) => <div key={`${line.timestamp}-${index}`} className="group grid grid-cols-[3rem_8rem_4rem_minmax(0,1fr)] gap-3 border-b border-border/60 px-4 py-2 hover:bg-muted/50"><span className="text-right text-muted-foreground">{index + 1}</span><span className="text-muted-foreground">{line.timestamp || "—"}</span><span className={`w-fit rounded px-1.5 py-0.5 text-[10px] font-bold ${levelStyle[line.level] ?? levelStyle.INFO}`}>{line.level}</span><span className="flex items-start gap-2 break-words text-foreground"><span className="flex-1">{line.message}</span><button onClick={() => { navigator.clipboard.writeText(line.message).catch(() => {}); setCopiedLine(index); setTimeout(() => setCopiedLine(null), 1500); }} className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" title="Copy line">{copiedLine === index ? <CopyCheck className="size-3 text-green-400" /> : <Copy className="size-3 text-muted-foreground" />}</button></span></div>)}<div ref={bottomRef} /></>}
    </div></ScrollArea>
  </section>;
}
