...
export default function ConfigEditor() {
  ...
  return <div className="flex h-full min-w-0 flex-col p-6">
    ...
    <div className="grid min-h-0 min-w-0 flex-1 grid-cols-1 gap-4 lg:grid-cols-[minmax(14rem,18rem)_minmax(0,1fr)]">
      ...
      {selectedConfig ? <section className={`grid min-h-0 min-w-0 gap-4 ${sections.length ? "xl:grid-cols-[minmax(0,1fr)_13rem]" : "grid-cols-1"}`}>
        <div className="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm">
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-border px-3 py-2"><div className="relative min-w-0 flex-1"><Search className="pointer-events-none absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" /><Input className="pl-8" value={findTerm} onChange={(event) => setFindTerm(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") findNext(); }} placeholder="Find in file" aria-label="Find in configuration file" /></div><Button size="sm" variant="outline" onClick={findNext} disabled={!findTerm.trim()}>Find</Button><Button size="sm" variant={wrapLines ? "secondary" : "outline"} onClick={() => setWrapLines(!wrapLines)} title="Toggle line wrapping" aria-pressed={wrapLines}><WrapText />Wrap</Button></div>
