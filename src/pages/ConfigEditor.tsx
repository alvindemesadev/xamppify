...
import { FileCog, Save, Search, WrapText } from "lucide-react";
import { getKnownConfigs, parseIniSections, readFile, writeFile } from "@/lib/ipc";
...
export default function ConfigEditor() {
  ...
  const saveMutation = useMutation({
    mutationFn: () => writeFile(selected!, content),
    onSuccess: () => {
  ...
  return <div className="flex h-full min-w-0 flex-col p-6">
    ...
    <div className="grid min-h-0 min-w-0 flex-1 grid-cols-1 gap-4 lg:grid-cols-[18rem_minmax(0,1fr)]">
      ...
      {selectedConfig ? <section className={`grid min-h-0 min-w-0 gap-4 ${sections.length ? "xl:grid-cols-[minmax(0,1fr)_13rem]" : "grid-cols-1"}`}>
        <div className="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border bg-muted/30 px-4 py-3"><div className="min-w-0"><p className="font-medium">{selectedConfig.name}</p><p className="truncate font-mono text-xs text-muted-foreground" title={selectedConfig.path}>{selectedConfig.path}</p></div><Button size="sm" onClick={() => setConfirmSave(true)} disabled={!hasUnsavedChanges || saveMutation.isPending}><Save />{saveMutation.isPending ? "Saving…" : "Save"}</Button></div>
    ...
    <ConfirmActionDialog open={confirmSave} onOpenChange={setConfirmSave} title="Save configuration changes?" description={`This will overwrite ${selectedConfig?.name ?? "the selected file"}. Incorrect settings can prevent XAMPP services from starting.`} confirmLabel="Save configuration" pending={saveMutation.isPending} onConfirm={() => { saveMutation.mutate(); setConfirmSave(false); }} />
  </div>;
}
